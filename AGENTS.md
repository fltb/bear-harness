...
## Character conversation quality validation

- [`docs/character-conversation-quality-checklist.md`](docs/character-conversation-quality-checklist.md) is the repository's acceptance rubric for whether a character sounds like a person rather than a generic assistant. It contains 72 scored checks across spoken naturalness, vocabulary, rhythm, turn structure, interaction, emotion, voice, desires and flaws, worldview, reasoning, continuity, and mode switching, plus 12 hard-failure conditions.
- Any material change to a character's stable identity, interaction rules, examples, prompt, Story, Canon, or character Skill behavior must be tested with real configured models. Unit fixtures and hand-written sample replies cannot substitute for original live-model output.
- A development retest uses at least two fresh Sessions and 24 unedited assistant replies per model. A completion or release claim uses at least four fresh Sessions and 48 replies per configured primary model, including one Session of at least 18 turns, repeated probes across clean Sessions, disagreement, correction after new evidence, refusal, topic switching, a real task, tool or memory boundaries, story entry/exit, and explicit OOC behavior.
- Score every configured primary model independently against all 72 checks. Passing requires `123/144` or higher, every category at `9/12` or higher, and no hard failure. The lowest model result controls the verdict; do not combine strong categories from different routes.
- Store the exact prompts and unedited assistant replies as machine-readable raw evidence under `docs/evaluations/`, together with model/provider, parameters, base commit, character source hash, Session boundaries, capture failures, mechanical statistics, item-level scores, cited turns, residual risks, and the verdict. Keep the before-change baseline when evaluating a rewrite.
- Prompts must not tell the model to "sound human", "be natural", "show a preference", or reveal the rubric. Examples added to the character package must not duplicate evaluation prompts verbatim. Failed transports may be retried only with a fresh Session; never splice successful turns from separate attempts into one accepted conversation.
- Release acceptance requires a second reviewer who has not read the character package to perform the replacement-name blind test before reading the package and checking consistency. A self-review by the author may be reported only as an engineering retest, not final release approval.
- Preserve related regression coverage for character loading and Skill trigger boundaries, and run the repository's existing live-model identity/memory test when the change touches voice, product boundaries, tools, or explicit memory. The reusable live-model harness is `apps/web-dev/e2e/jizhou-character-quality-live.spec.ts`; its current adversarial corpus and full pass/fail evidence are documented in `docs/evaluations/jizhou-gpt-5.6-2026-09-12-adversarial.md`. The earlier aligned-prompt run remains in `docs/evaluations/jizhou-gpt-5.6-2026-09-12-after.md` as a retracted acceptance claim, not the reference verdict.
...
- Release from a clean tree with a non-placeholder version and verifiable attestations. Public distribution may use platform-unsigned packages; Apple Developer ID signing/notarization and Windows Authenticode signing are optional, not release gates.
- Public releases include SHA256SUMS.txt and its detached OpenPGP signature, with the public verification key and full fingerprint published for users. GPG verifies release provenance and integrity; it does not replace OS-trusted code signing or suppress Gatekeeper/SmartScreen warnings. Release notes must disclose unsigned/unnotarized status and installation implications. RC tags publish as prereleases, not mandatory drafts.
- Release signing private keys and passphrases never enter the repository, artifacts, logs, or chat. Keep protected backups outside the repository and provide CI signing credentials through GitHub Actions secrets; only public keys and fingerprints belong in version control.
