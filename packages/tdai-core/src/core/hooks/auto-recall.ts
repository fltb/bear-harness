...
async function searchMemories(
	...
	try {
		...
		const nativeHybridSearch = vectorStore?.searchL1Hybrid;
		if (vectorStore?.getCapabilities().nativeHybridSearch && nativeHybridSearch) {
			const tNative = performance.now();
			const results = await nativeHybridSearch.call(vectorStore, {
				query: cleanText,
				topK: maxResults,
			});
		...
		if (ftsQuery) {
			...
			if (ftsResults.length > 0) {
				...
				if (ftsResults.length <= maxResults) {
					...
					return ftsResults
						.slice(0, maxResults)
						.map((r) => formatMemoryLine(ftsResultToFormatable(r)));
				}
				logger?.debug?.(
...
async function searchByEmbedding(
	...
	logger?.debug?.(
		`${TAG} [embedding-search] Got ${vecResults.length} candidates, filtering by threshold=${threshold}`,
	);
...
function recordToFormatable(record: MemoryRecord): FormatableMemory {
	const meta = record.metadata as
		| { activity_start_time?: string; activity_end_time?: string }
		| undefined;
	return {
		type: record.type,
...
function vectorResultToFormatable(r: L1SearchResult): FormatableMemory {
	...
	if (r.metadata_json && r.metadata_json !== "{}") {
		try {
			const meta =
				typeof r.metadata_json === "string" ? JSON.parse(r.metadata_json) : r.metadata_json;
