...
export async function performAutoCapture(params: {
	...
	try {
		await checkpoint.captureAtomically(sessionKey, pluginStartTimestamp, async (afterTimestamp) => {
			...
			logger?.debug?.(
				`${TAG} L0 recorded: ${filteredMessages.length} messages for session ${sessionKey}`,
			);
		...
		logger?.error(
			`${TAG} L0 recording failed: ${err instanceof Error ? err.message : String(err)}`,
		);
	}
	...
	if (filteredMessages.length > 0 && vectorStore) {
		...
		for (let i = 0; i < filteredMessages.length; i++) {
			...
			try {
				...
				const upsertOk = await vectorStore.upsertL0(
					l0Record,
					supportsBgEmbed ? undefined : embedding,
				);
			...
			const bgPromise: Promise<void> = (async () => {
				...
				try {
					...
					for (let i = 0; i < bgSnapshot.length; i++) {
						try {
							const ok = await bgVectorStore.updateL0Embedding!(
