...
export async function extractL1Memories(params: {
	...
	logger?.debug?.(
		`${TAG} Total extracted memories: ${allExtracted.length} across ${scenes.length} scene(s)`,
	);
	...
	if (extracted.length > maxMemoriesPerSession) {
		logger?.debug?.(
			`${TAG} Limiting from ${extracted.length} to ${maxMemoriesPerSession} memories per session`,
		);
	...
	logger?.info(
		`${TAG} Extraction complete: extracted=${extracted.length}, stored=${storedRecords.length}`,
	);
