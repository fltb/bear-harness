...
export class SceneExtractor {
	...
	constructor(opts: SceneExtractorOptions) {
		...
		if (!opts.llmRunner) {
			throw new Error("TdaiCore requires a HostAdapter LLMRunner for scene extraction");
		}
	...
	async extract(
		...
		try {
			...
			this.logger?.debug?.(
				`${TAG} extract() persona.md navigation updated: ${Date.now() - navStartMs}ms`,
			);
		} catch (navErr) {
	...
	private async updateSceneNavigation(): Promise<void> {
		...
		try {
			...
			this.logger?.debug?.(
				`${TAG} updateSceneNavigation() skipped: no persona file yet, waiting for PersonaGenerator`,
			);
			return;
		}
