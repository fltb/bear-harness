...
export class VectorStore implements IMemoryStore {
	...
	private tableRowCount(table: string): number {
		...
		try {
			const row = this.db.prepare(`SELECT COUNT(*) AS cnt FROM ${table}`).get() as
				| { cnt: number }
				| undefined;
			return row?.cnt ?? 0;
		} catch {
	...
	deleteL1Expired(cutoffIso: string): number {
		...
		try {
			...
			const totalRow = this.db.prepare("SELECT COUNT(*) AS cnt FROM l1_records").get() as {
				cnt: number;
			};
			...
			try {
				if (this.vecTablesReady) {
					this.db
						.prepare("DELETE FROM l1_vec WHERE updated_time != '' AND updated_time < ?")
						.run(cutoffIso);
				}
				this.db
					.prepare("DELETE FROM l1_records WHERE updated_time != '' AND updated_time < ?")
					.run(cutoffIso);
				this.db.exec("COMMIT");
				this.logger?.info?.(
	...
	countL1(): number {
		...
		try {
			const row = this.db.prepare("SELECT COUNT(*) AS cnt FROM l1_records").get() as {
				cnt: number;
			};
	...
	queryL1Records(filter?: L1QueryFilter): L1RecordRow[] {
		...
		try {
			...
			if (sessionId && updatedAfter) {
				raw = this.stmtQueryBySessionIdSince.all(sessionId, updatedAfter) as Record<
					string,
					unknown
				>[];
			} else if (sessionId) {
				...
				raw = this.stmtQueryBySessionKeySince.all(sessionKey, updatedAfter) as Record<
					string,
					unknown
				>[];
			} else if (sessionKey) {
	...
	deleteL0Expired(cutoffIso: string): number {
		...
		try {
			...
			// Ratio protection: refuse to delete > 80% in one pass
			const totalRow = this.db.prepare("SELECT COUNT(*) AS cnt FROM l0_conversations").get() as {
				cnt: number;
			};
			const total = totalRow.cnt;
			const ratio = total > 0 ? expiredCount / total : 0;
			...
			try {
				if (this.vecTablesReady) {
					this.db
						.prepare("DELETE FROM l0_vec WHERE recorded_at != '' AND recorded_at < ?")
						.run(cutoffIso);
				}
				this.db
					.prepare("DELETE FROM l0_conversations WHERE recorded_at != '' AND recorded_at < ?")
					.run(cutoffIso);
				this.db.exec("COMMIT");
				this.logger?.info?.(
	...
	countL0(): number {
		if (this.degraded) return 0;
		try {
			const row = this.db.prepare("SELECT COUNT(*) AS cnt FROM l0_conversations").get() as {
				cnt: number;
			};
	...
	queryL0ForL1(
		...
		try {
			...
			if (afterRecordedAtMs && afterRecordedAtMs > 0) {
				...
				rows = this.stmtL0QueryAfter.all(sessionKey, afterRecordedAtIso, limit) as Array<
