...
export class TcvdbClient {
	...
	async request<T = ApiResponse>(path: string, body: Record<string, unknown>): Promise<T> {
		...
		for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
			...
			try {
				this.logger?.debug?.(
					`${TAG} → ${path} attempt=${attempt} body=${JSON.stringify(body).slice(0, 500)}`,
				);
				...
				this.logger?.info(
					`${TAG} ${path} ${totalMs}ms${attempt > 0 ? ` (${attempt + 1} attempts)` : ""}`,
				);
		...
		const totalMs = Math.round(performance.now() - t0);
		this.logger?.debug?.(
			`${TAG} ✗ ${path} totalMs=${totalMs} attempts=${MAX_RETRIES + 1} error=${lastError?.message}`,
		);
