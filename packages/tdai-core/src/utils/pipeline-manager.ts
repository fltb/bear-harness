...
export type PipelineStatePersister = (
...
export class MemoryPipelineManager {
	...
	private advanceWarmupThreshold(state: PipelineSessionState): void {
		...
		if (next >= this.everyNConversations) {
			...
			this.logger?.debug?.(
				`${TAG} Warm-up graduated → using steady-state threshold ${this.everyNConversations}`,
			);
		} else {
	...
	private async _doFlush(): Promise<void> {
		...
		for (const [sessionKey, timers] of this.sessionTimers) {
			if (timers.l1Idle.pending) {
				...
				if (buffer && buffer.length > 0) {
					this.logger?.debug?.(
						`${TAG} [${sessionKey}] Flush: enqueuing L1 for ${buffer.length} buffered messages`,
					);
					this.enqueueL1(sessionKey, "flush");
				}
			}
		...
		this.logger?.debug?.(
			`${TAG} Waiting for queues to drain (l2=${this.l2Queue.size}, l3=${this.l3Queue.size})`,
		);
	...
	private enqueueL1(
		sessionKey: string,
		triggerReason: "threshold" | "idle_timeout" | "flush" = "threshold",
	): void {
		const timers = this.getOrCreateTimers(sessionKey);
	...
	private async runL1(sessionKey: string): Promise<void> {
		...
		if (buffer.length === 0 && state.conversation_count === 0) {
			this.logger?.debug?.(
				`${TAG} [${sessionKey}] L1 skipped: no messages and no pending conversations`,
			);
			return;
		}
	...
	private enqueueL2(sessionKey: string, trigger: string): void {
		...
		this.logger?.debug?.(
			`${TAG} [${sessionKey}] Enqueuing L2 (trigger=${trigger}, queue=${this.l2Queue.name})`,
		);
