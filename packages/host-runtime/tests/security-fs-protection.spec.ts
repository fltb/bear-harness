...
function protect(roots: string[], onHit?: (hit: FsProtectionHit) => void) {
	const handle = installFsProtection({ protectedRoots: roots, onHit });
	handles.push(handle);
	return handle;
}
...
describe("installFsProtection", () => {
	it("warns and records hits for deletes inside a protected root (pass-through)", () => {
		const root = mkdtempSync(join(tmpdir(), "fs-protect-"));
		const target = join(root, "inside.txt");
		writeFileSync(target, "x");
		const hits: FsProtectionHit[] = [];
		const warns: string[] = [];
		const handle = installFsProtection({
			protectedRoots: [root],
	...
	it("covers rmdirSync and the promises API with the right operations", async () => {
		...
		fs.mkdirSync(dir);
		const hits: FsProtectionHit[] = [];
		protect([root], (hit) => hits.push(hit));
	...
	it("uninstall restores the original fs functions", () => {
		...
		writeFileSync(target, "x");
		let hits = 0;
		const second = installFsProtection({ protectedRoots: [root], onHit: () => (hits += 1) });
	...
	it("is idempotent: a second install returns the same handle and never double-wraps", () => {
		const root = mkdtempSync(join(tmpdir(), "fs-protect-"));
		let hits = 0;
		const first = installFsProtection({ protectedRoots: [root], onHit: () => (hits += 1) });
		handles.push(first);
		const second = installFsProtection({ protectedRoots: [root], onHit: () => (hits += 1) });
		expect(second).toBe(first);
		const third = installFsProtection({ protectedRoots: [root] });
