...
} from "../src/memory/presentation-store.js";
...
describe("MemoryPresentationStore", () => {
	...
	it("marks a memory invalidated even when no replacement is supplied", () => {
		const database = openDatabase();
		const store = new MemoryPresentationStore(database.orm);
		store.recordDirectCreation(scopeA, {
			backendMemoryId: "forgotten-memory",
			createdBy: "imported",
		});

		store.recordReplacement(scopeA, "forgotten-memory");

		expect(store.get(scopeA, "forgotten-memory")).toMatchObject({
			backendMemoryId: "forgotten-memory",
			invalidatedAt: expect.any(String),
		});
		expect(store.get(scopeA, "forgotten-memory")).not.toHaveProperty("replacementMemoryId");
	});
	...
	it("deletes metadata only for the forgotten scoped backend record and lists requested IDs", () => {
		...
		expect(
			store.list(scopeA, ["missing", "memory-2", "memory-1"]).map((row) => row.backendMemoryId),
	...
	it("has no content or body-text column and never persists a memory body", () => {
		...
		const rows = database.connection.prepare("SELECT * FROM memory_presentation").all() as Array<
