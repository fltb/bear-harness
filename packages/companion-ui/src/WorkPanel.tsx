...
const interruptible = (status: RunInfo["status"]) =>
	status === "enqueued" || status === "running" || status === "needs_user";
const isFailure = (status: RunInfo["status"]) =>
	status === "failed" || status === "cancelled" || status === "forced_termination";
	...
	const start = () =>
		void workflow.runCommissionAction(props.commission.id, async () => {
			if (props.commission.status === "draft")
				await workflow.host.commission.approve(props.commission.id, props.commission.draft.hash);
			await workflow.host.commission.launch(props.commission.id, "pi-product-managed");
		});
	const reject = () =>
		void workflow.runCommissionAction(props.commission.id, () =>
			workflow.host.commission.reject(props.commission.id),
		);
	return (
		<div class="action-proposal" data-commission-id={props.commission.id}>
			...
			<div class="scope-list">
				<Show when={props.commission.draft.resourceGrants.length > 0}>
					<p>
						<strong>{t("work.reads")}</strong>
						{props.commission.draft.resourceGrants
							.map((grant) => `${grant.resourceId} (${grant.operations.join("/")})`)
							.join("、")}
					</p>
				</Show>
				<Show when={props.commission.draft.outputGrants.length > 0}>
					<p>
						<strong>{t("work.writes")}</strong>
						{props.commission.draft.outputGrants
							.map((grant) => `${grant.parentResourceId}/${grant.relativePath ?? "."}`)
							.join("、")}
					</p>
				</Show>
				<p>
					<strong>{t("work.network")}</strong>
					{props.commission.draft.networkPolicy.allowed
						? t("work.networkYes")
						: t("work.networkNo")}
				</p>
				<p>{t("work.externalAppWarning")}</p>
			</div>
			<div class="work-actions">
				<Button data-control="command" type="button" disabled={actionState.busy()} onClick={start}>
					{props.labels?.approve ?? t("work.timeline.start")}
				</Button>
				<Button data-control="command" type="button" disabled={actionState.busy()} onClick={reject}>
...
export function PermissionCard(props: { permission: RunPermissionRequest }) {
	...
	const runAction = (action: () => Promise<unknown>) =>
		workflow.runPermissionAction(actionId, action);
	return (
		<div class="action-proposal needs-user" data-permission-request={props.permission.requestId}>
			...
			<div class="work-actions">
				...
				<Button
					...
					type="button"
					disabled={actionState.busy()}
					onClick={() => runAction(() => workflow.host.run.cancel(props.permission.runId))}
				>
...
export function WorkRunCard(props: {
	...
	const submitSteer = async () => {
		...
		if (
			await workflow.runRunAction(props.run.id, () =>
				workflow.host.run.steer(props.run.id, instruction),
			)
		)
			actionState.setSteerText("");
	};
		...
		resultSpace.open(
			{
				...
				triggerEntryId: props.messageId,
	...
	return (
		...
		>
				...
				<div class="steer-row">
					<TextField>
						<TextField.Input
							...
							value={actionState.steerText()}
							onInput={(event) => actionState.setSteerText(event.currentTarget.value)}
							onKeyDown={(event) => {
								if (event.key === "Enter") {
					...
					<Button
						data-control="command"
						type="button"
						disabled={actionState.busy() || !actionState.steerText().trim()}
				...
				<div class="work-actions">
					...
					<Show when={props.run.status === "interrupted"}>
						<Button
							...
							onClick={() =>
								void workflow.runRunAction(props.run.id, () =>
									workflow.host.run.resume(props.run.id),
								)
							}
						>
			...
			<Show when={props.run.status === "completed" && runArtifacts().length > 0}>
				<div class="completion-card">
					...
					<p class="completion-count">
						{t("work.timeline.resultCount").replace("{count}", String(runArtifacts().length))}
					</p>
					<div class="artifact-list">
							...
							{(artifact) => (
								<div class="artifact-row">
									...
									<Button
										data-control="command"
										type="button"
										onClick={() => void workflow.host.artifact.download(artifact.id)}
									>
										{t("work.download")}
									</Button>
								</div>
			...
			<Show when={runArtifacts().length > 0}>
