...
export function MemoryEntryList(props: {
	...
	const togglePin = (entry: MemoryEntry) => () =>
		runEntryAction(
			...
			entry.pinned ? "已取消置顶" : "已置顶",
		);
	const forget = (entry: MemoryEntry) => () =>
		runEntryAction(entry.id, () => store.memory.forget(entry.id), "已忘记这条记忆");
	const exclude = (entry: MemoryEntry) => () =>
		runEntryAction(
			entry.id,
			() => store.memory.exclude(entry.id, true),
			"已从回忆中排除",
		);

	const title = () => props.title ?? "已记住";
	...
	return (
		<section class="memory-section" aria-label={title()}>
			...
			<Show when={!loading() && !error() && entries().length === 0}>
				<p class="empty-note">还没有记住什么。确认“最近想记住”里的候选后，会出现在这里。</p>
			</Show>
					...
					{(entry) => (
						<li class="memory-entry" data-pinned={entry.pinned || undefined}>
							...
							<div class="memory-meta">
								...
								<span>{entry.sourceConversationTitle || "对话"}</span>
...
export function MemorySheet() {
	...
	async function decide(candidateId: string, decision: MemoryDecision): Promise<void> {
		...
		try {
			...
			setFeedback(
				decision === "approve" ? "已记住" : decision === "reject" ? "已拒绝" : "已保存",
			);
			setEditingId(null);
	...
	return (
		<div class="sheet-panel">
			<section class="candidates-section" aria-label="最近想记住">
				<div class="section-head">
					<h3>最近想记住</h3>
				...
				<Show when={!candidatesLoading() && !candidatesError() && pendingCandidates().length === 0}>
					<p class="empty-note">没有待确认的候选记忆。</p>
				</Show>
