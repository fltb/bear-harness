 ...
 *
 * The bridge exposes `voice.list` and `voice.switch`; state arrives from
 * the `list` response, the boot snapshot, and `voice.*` domain events
 * (`voice.stack_switched` / `voice.stack_pinned`), which re-fetch the list
 * so the active stack is always current.
 *
...
export function createVoiceStore(): VoiceStore {
	const [resource, actions] = createResource<VoiceListData>(
		() => invoke(() => window.bearDesktop.companion.voice.list()),
	...
	return {
		...
		refetch,
		list: () => invoke(() => window.bearDesktop.companion.voice.list()),
		switch: async (stackId, scope) => {
			await invoke<void>(() => window.bearDesktop.companion.voice.switch(stackId, scope));
			refetch();
		},
		_hydrate: (value) => {
