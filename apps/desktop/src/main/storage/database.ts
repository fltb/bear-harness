...
/** Open (or retrieve) the canonical database connection. */
export function openDatabase(databaseDir: string): Database {
	if (db) return db;

	dbPath = join(databaseDir, "canon.db");
	backupDir = join(databaseDir, "schema-backups");
...
export function currentVersion(): number {
	...
	const row = db.prepare("SELECT COALESCE(MAX(id), 0) AS v FROM schema_migrations").get() as { v: number };
	return row.v;
}
...
function backupSchema(): string {
	const timestamp = Date.now().toString(36);
	const backupPath = join(backupDir, `canon-${timestamp}.db`);
...
function applyMigration(migration: Migration): void {
	...
	const chk = checksum(migration.up);
	...
	try {
		...
		db.prepare("INSERT INTO schema_migrations (id, checksum) VALUES (?, ?)").run(migration.id, chk);
		db.exec("COMMIT");
	} catch (e) {
		db.exec("ROLLBACK");
		throw new Error(
...
export function recordLatency(operation: string, elapsedMs: number): void {
