...
function isMalicious(magic: FileTypeResult | undefined, extension: string): boolean {
	...
	if (
		(extKind === "pdf" || extKind === "docx" || extKind === "xlsx" || extKind === "pptx" || extKind === "image") &&
		magicKind === "unknown" &&
		magic.ext !== "zip" &&
		magic.ext !== "cfb"
	) {
...
function isEncrypted(buffer: Buffer, magic: FileTypeResult | undefined, extension: string): boolean {
...
export class IngestService {
	 ...
	 */
	async ingest(params: { buffer: Buffer; logicalName: string; signal?: AbortSignal }): Promise<MaterialRef> {
