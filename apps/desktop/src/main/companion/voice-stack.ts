...
export class VoiceStackManager {
	...
	rollbackAvailable(companionId: string): boolean {
		...
		const older = this.db
			.prepare("SELECT id FROM voice_stack_versions WHERE companion_id = ? AND revision < ? LIMIT 1")
	...
	private companionExists(companionId: string): boolean {
		return (
			this.db.prepare("SELECT id FROM companion_identity WHERE id = ?").get(companionId) !== undefined
		);
	}
