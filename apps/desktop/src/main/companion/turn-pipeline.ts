...
export class TurnPipeline {
	...
	async sendUserMessage(conversationId: string, text: string): Promise<TurnResult> {
		...
		if (this.activeTurns.get(conversationId)) {
		...
		const branchRow = this.db
			.prepare("SELECT id FROM branches WHERE conversation_id = ? AND adopted = 1 ORDER BY created_at DESC LIMIT 1")
		...
		try {
			this.db
				.prepare("INSERT INTO messages (id, conversation_id, branch_id, role) VALUES (?, ?, ?, 'user')")
		...
		this.activeTurns.set(conversationId, true);
		this.eventBus.publish("message.user_sent", { conversationId, messageId, versionId });
	...
	async abort(conversationId: string): Promise<void> {
		if (!this.activeTurns.get(conversationId)) return;
		this.supervisor.sendCommand({ type: "abort", conversationId });
		this.activeTurns.set(conversationId, false);
	...
	async regenerate(conversationId: string, messageId: string): Promise<TurnResult> {
		...
		// Create a sibling assistant message version
		const newVersionId = randomUUID();
		this.db
			.prepare(
				"UPDATE message_versions SET adopted = 0 WHERE message_id = ?",
			)
			.run(messageId);
		this.db
			.prepare(
		...
		this.eventBus.publish("message.regenerated", { conversationId, messageId, versionId: newVersionId });
	...
	hasActiveTurn(conversationId: string): boolean {
		return this.activeTurns.get(conversationId) ?? false;
	}
}