...
const FSOP_KINDS: readonly FsopKind[] = ["create", "write_new", "convert", "mkdir", "move", "rename"];
const NEEDS_SRC: readonly FsopKind[] = ["write_new", "convert", "move", "rename"];
const NEEDS_DST: readonly FsopKind[] = ["create", "write_new", "convert", "mkdir", "move", "rename"];
...
export class FileOpsService {
	...
	private validateOpPaths(op: FsopOp, roots: string[]): void {
		if (NEEDS_DST.includes(op.kind) && op.dst === undefined) fail("invalid_request", "op_missing_dst");
		if (NEEDS_SRC.includes(op.kind) && op.src === undefined) fail("invalid_request", "op_missing_src");
	...
	private executeOp(
		...
		try {
			...
			switch (op.kind) {
				case "create": {
					const content =
						op.contentBase64 !== undefined ? Buffer.from(op.contentBase64, "base64") : Buffer.alloc(0);
	...
	private undoOne(entry: FsopJournal): FsopJournal {
		...
		try {
			...
			return { ...base, status: "error", error: e instanceof Error ? e.message : JSON.stringify(e) };
		}
	}
