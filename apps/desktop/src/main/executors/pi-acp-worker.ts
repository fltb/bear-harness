...
class PiAcpAgent {
	...
	private writeTool(sessionId: string) {
		return {
			...
			description: "Create or replace a text file only when the approved action explicitly allows it.",
			parameters: Type.Object({
				path: Type.String({ minLength: 1 }),
				content: Type.String(),
			}),
	...
	private async forwardPiEvent(sessionId: string, event: { type: string; [key: string]: unknown }): Promise<void> {
...
async function selectConfiguredModel(runtime: PiModelRuntime, session: AgentSession): Promise<boolean> {
	...
	if ("content" in value && Array.isArray(value.content)) {
		return value.content
			.filter((part): part is { type: string; text: string } =>
				Boolean(part && typeof part === "object" && "type" in part && "text" in part && typeof part.text === "string"),
			)
