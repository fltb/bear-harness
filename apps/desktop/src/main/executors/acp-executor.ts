...
import { AcpRunClient, type AcpPermissionRequest, type AcpProcessSpec } from "./acp-client.js";
import type {
	ExecutorController,
...
export class ApprovedFileAccess {
	...
	async readTextFile(request: acp.ReadTextFileRequest): Promise<acp.ReadTextFileResponse> {
		...
		const sliced = content.split(/\r?\n/).slice(line - 1, line - 1 + Math.min(limit, 2000)).join("\n");
	...
	async writeTextFile(request: acp.WriteTextFileRequest): Promise<acp.WriteTextFileResponse> {
		...
		this.record("acp.file_write", { path: target, bytes: Buffer.byteLength(request.content, "utf8") });
		return {};
	}
	...
	private resolveWritable(path: string): string {
		...
		const permitted = this.writeRoots.some((root) => {
			...
			return root.lexical === target && isInside(realpathSync(dirname(root.lexical)), canonicalParent);
		});
		if (!permitted) throw new Error("approved_path_symlink_escape");
		return target;
	}

	private assertLexicallyAllowed(path: string, roots: AllowedRoot[], access: "read" | "write"): string {
	...
	private async runPrompt(active: ActiveRun): Promise<void> {
		try {
			...
			if (response.stopReason === "cancelled") {
				...
				this.settle(active, { type: "completed", summary: active.messageParts.join("").trim() || undefined });
			}
		} catch (error) {
...
function toAllowedRoot(path: string, access: "read" | "write"): AllowedRoot {
	if (!isAbsolute(path)) throw { kind: "validation_failed", reason: `executor_${access}_root_not_absolute` };
