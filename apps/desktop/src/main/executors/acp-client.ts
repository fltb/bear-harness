...
export class AcpRunClient {
	...
	private createClientApp(): acp.ClientApp {
		...
		if (this.handlers.readTextFile) {
			app.onRequest(acp.methods.client.fs.readTextFile, (ctx) => this.handlers.readTextFile!(ctx.params));
		}
		if (this.handlers.writeTextFile) {
			app.onRequest(acp.methods.client.fs.writeTextFile, (ctx) => this.handlers.writeTextFile!(ctx.params));
		}
		if (this.handlers.createTerminal) {
			app.onRequest(acp.methods.client.terminal.create, (ctx) => this.handlers.createTerminal!(ctx.params));
		}
		if (this.handlers.terminalOutput) {
			app.onRequest(acp.methods.client.terminal.output, (ctx) => this.handlers.terminalOutput!(ctx.params));
		}
		...
		if (this.handlers.killTerminal) {
			app.onRequest(acp.methods.client.terminal.kill, (ctx) => this.handlers.killTerminal!(ctx.params));
		}
		if (this.handlers.releaseTerminal) {
			app.onRequest(acp.methods.client.terminal.release, (ctx) => this.handlers.releaseTerminal!(ctx.params));
		}
		return app;
	}
