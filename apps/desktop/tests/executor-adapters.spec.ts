...
import { CodexAdapter } from "../src/main/executors/codex-adapter.js";
import { PiAcpAdapter } from "../src/main/executors/pi-adapter.js";
import { EventBus } from "../src/main/storage/event-bus.js";
import type { AcpProcessSpec } from "../src/main/executors/acp-client.js";
...
describe("ACP executor adapters", () => {
	it("launches Pi as a dedicated ACP worker and records no secret manifest data", async () => {
		...
		const run = request(cwd, { id: "pi-product-managed", type: "product-managed", capabilities: {} });
		run.emit = (event) => {
	...
	it("routes a re-verified consented Codex profile through ACP", async () => {
		...
		expect(
			(db.prepare("SELECT kind FROM events").all() as Array<{ kind: string }>).map((event) => event.kind),
