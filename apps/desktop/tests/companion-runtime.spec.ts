...
} from "../src/main/companion/supervisor.js";
...
describe("in-process Companion Host bridge", () => {
	...
	it("routes a role plugin Host request directly to the allowlisted Host handler", async () => {
		...
		db.exec("CREATE TABLE events (seq INTEGER PRIMARY KEY, kind TEXT NOT NULL, payload TEXT NOT NULL)");
		...
		runtime.sendCommand({ type: "prompt", conversationId: "conversation-1", message: "切换到雪原" });
		await settleRuntime();
	...
	it("compiles the role prompt and exposes only its controlled tools", async () => {
		...
		await runtime.start();
		runtime.sendCommand({ type: "prompt", conversationId: "conversation-2", message: "查看站点记录" });
		...
		const runtimeReady = db
			.prepare("SELECT payload FROM events WHERE kind = 'companion.runtime_ready' ORDER BY seq DESC LIMIT 1")
