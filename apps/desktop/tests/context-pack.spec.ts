...
import { ContextPackCompiler } from "../src/main/companion/context-pack.js";
import { loadCharacter } from "../src/main/companion/character-loader.js";
...
describe("ContextPackCompiler character package identity", () => {
	it("injects identity_core from the active package without a product fallback", () => {
		...
		const pack = new ContextPackCompiler(db).compile("conversation-1");
		expect(pack.blocks.find((block) => block.layer === "identity")?.content).toBe(
			character.identity_core,
		);
		expect(() => new ContextPackCompiler(db).compile("missing-conversation")).toThrow(
			"conversation has no companion identity",
		);
		db.close();
	});
});
