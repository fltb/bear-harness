...
import { IngestService, sanitizeName, sniffKind } from "../../src/main/materials/ingest.js";
...
describe("sniffKind", () => {
	it("maps declared MIME types", () => {
		...
		expect(
			sniffKind("application/vnd.openxmlformats-officedocument.presentationml.presentation", "pptx"),
		).toBe("pptx");
...
describe("IngestService", () => {
	...
	it("ingests plain text and markdown", async () => {
		...
		expect(text).toMatchObject({ kind: "text", logicalName: "a.txt", mime: "text/plain", state: "ready" });
	...
	it("detects text without a magic signature via extension and content", async () => {
		...
		const heur = await service.ingest({ buffer: Buffer.from("plain text, unknown extension"), logicalName: "note.notes" });
	...
	it("marks encrypted zips as encrypted", async () => {
		const ref = await service.ingest({ buffer: await makeEncryptedZip(), logicalName: "secret.zip" });
	...
	it("marks unsupported kinds as unsupported", async () => {
		const zip = new JSZip();
		zip.file("a.txt", "hi");
		const ref = await service.ingest({ buffer: await zip.generateAsync({ type: "nodebuffer" }), logicalName: "bundle.zip" });
