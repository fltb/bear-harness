...
原生 Pi 事件经过最小安全投影后进入 transient subscriber 集合。事件不落库；快照由对应 `AgentSession` 直接读取。
...
产品域变更只发进程内失效通知，不保存、不回放；审计单独写入审计日志。列表接口轻量且可分页；conversation、Run、Artifact 和 Character/Display 通过按 ID detail 读取。bootstrap 不扫描所有会话。
