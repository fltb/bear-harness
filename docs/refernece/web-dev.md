...
| `POST /diagnostics/renderer-fault` | Token required | Validates a bounded fault payload, joins a valid renderer traceparent or restarts the trace, and writes an allowlisted local diagnostic record; responds `204`. |
...
Renderer fault transport remains intentionally fire-and-forget: the client does not await, retry, or surface the fetch result. The Host now validates and persists accepted fault metadata in its local diagnostics JSONL; it never accepts message/stack content through this route and never uploads records. The same diagnostics root also stores catalog-only business traces. `BEAR_LOG_LEVEL` defaults to `info`; source-only `trace` enables redacted content evidence, while packaged apps clamp `trace` off. Maintainers can query/export one trace locally through the host-runtime diagnostics API or the root `diagnostics:export` script.
