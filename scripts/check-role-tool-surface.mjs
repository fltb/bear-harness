...
if (allowedHostTools.size + 1 > 12)
	failures.push("model tool surface exceeds 12 tools including role_skill");
if (failures.length) {
	console.error(`Role tool-surface violations:\n${failures.join("\n")}`);
	process.exit(1);
}
console.log("Role tool surface valid: role_skill plus 11 conditional Host domain tools");
